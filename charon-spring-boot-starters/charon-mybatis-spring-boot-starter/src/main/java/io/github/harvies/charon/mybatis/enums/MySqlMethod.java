package io.github.harvies.charon.mybatis.enums;
...
public enum MySqlMethod {
    ...
    @Deprecated
    PUT_FEATURE("putFeature", "根据id往features字段写入数据", "<script> UPDATE %s SET features = json_set(IFNULL(features,'{}'),'$.${key}', #{value}) WHERE %s=#{%s} </script>"),
    ...
    PUT_JSONS("putJsons", "根据id List往json字段写入多个key value数据", "" +
            ...
            "; </script>"),
    @Deprecated
    PUT_FEATURES("putFeatures", "根据id往features字段写入多个key value数据", "" +
            "<script> UPDATE " +
            "            %s " +
            "        SET " +
            "            features = json_set(IFNULL(features,'{}'), " +
            "                    <foreach item=\"value\" index=\"key\" collection=\"values.entrySet()\" separator=\",\"> " +
            "                        CONCAT(\"$.\", JSON_QUOTE(\"${key}\")), #{value} " +
            "                    </foreach> " +
            "                ) " +
    ...
    @Deprecated
    GET_FEATURE("getFeature", "根据id和key从features字段查询数据", "<script> SELECT JSON_EXTRACT(features, '$.${key}') FROM %s WHERE %s=#{%s} </script>"),
    ...
    @Deprecated
    GET_FEATURES("getFeatures", "根据id和keys从features字段查询数据", "<script> SELECT " +
            " <foreach item=\"value\" index=\"key\" collection=\"keys\" separator=\",\"> " +
            "    JSON_EXTRACT(features, '$.${value}') as ${value} " +
            " </foreach>" +
            " FROM %s WHERE %s=#{%s} </script>"),
